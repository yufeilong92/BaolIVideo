 ...
 * @Title: PolyvPlayerDanmuFragment.java
...
public class PolyvPlayerDanmuFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.polyv_fragment_player_danmu, null);
        initView();
        return view;
    }
    ...
    private void initView() {
        ...
        danmakuManager = new PolyvDanmakuManager(getActivity());
        //-------------------仅对加载的弹幕有效-------------------//
        // 设置最大显示行数
        ...
        getDanmakuListener = new PolyvDanmakuManager.GetDanmakuListener(){
            ...
            @Override
            public void success(BaseDanmakuParser danmakuParser, PolyvDanmakuEntity danmakuEntity) {
                toastMsg("获取弹幕成功，总数" + danmakuEntity.getAllDanmaku().size());
                if (iDanmakuView != null)
                    iDanmakuView.prepare(danmakuParser, mContext);
            }
        };
        sendDanmakuListener = new PolyvDanmakuManager.SendDanmakuListener() {
            ...
            @Override
            public void success(String responseMsg) {
                toastMsg("发送成功");
            }
        };
